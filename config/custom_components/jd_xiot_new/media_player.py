...
from .entities.mediaplayer.jd_media_player_mapping import create_media_player
...
async def async_setup_entry(
    ...
    _LOGGER.info("Set up the media player platform")
    ...
    for detail in details:
        ...
        entity = create_media_player(device_type, client, detail)
        if entity is not None:
            _LOGGER.info("Add media player: %s", device_type)
            entities.append(entity)
        else:
            _LOGGER.info(
                "Skipping non-media-player device %s (type: %s)",
                detail.get("did"),
                device_type,
            )

    if not entities:
        _LOGGER.warning("No media player devices found among selected device IDs")

...
def joy_device_additional_decode(data: dict[str, Any]) -> JoyDeviceAdditional:
    ...
    if not isinstance(name, str):
        raise TypeError(f"name 必须是字符串，实际类型：{type(name)}")

    if not isinstance(productId, int):
        raise TypeError(f"productId 必须是整型，实际类型：{type(productId)}")

    if not isinstance(modelId, int):
        raise TypeError(f"modelId 必须是整型，实际类型：{type(modelId)}")

    if not isinstance(userDeviceId, int):
        raise TypeError(f"userDeviceId 必须是整型，实际类型：{type(userDeviceId)}")

...
async def async_setup_entry(
    ...
    _LOGGER.info("Set up the cover platform")
    ...
    for detail in details:
        ...
        entity = create_cover_entity(device_type, client, detail)
        if entity is not None:
            _LOGGER.info("Add cover: %s", device_type)
            entities.append(entity)
        else:
            _LOGGER.info(
                "Skipping non-cover device %s (type: %s)",
                detail.get("did"),
                device_type,
            )

    if not entities:
        _LOGGER.warning("No cover devices found among selected device IDs")

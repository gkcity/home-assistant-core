...
from custom_components.jd_xiot.entities.mediaplayer.jd_media_player_mapping import (
    register_media_player_entity,
)
from xiot_core.support.typedef.controller.device_controller import DeviceController
from xiot_core_device_controller.jd._gateway._jdzhp02qs.device_jdzhp02qs import (
    DeviceJdzhp02qs,
)
...
@register_media_player_entity
class DeviceJdzhp02qsEntity(MediaPlayerEntity):
    """DeviceJdzhp02qs Entity."""

    TYPE: str = DeviceJdzhp02qs.TYPE
    ...
    def __init__(
        ...
        self._device: DeviceJdzhp02qs | None = None

        # 必须：声明实体唯一 ID（不能重复，用于 HA 识别设备）
        self._attr_unique_id: str = f"jd_xiot_{controller.did}"
        ...
        # 绑定设备控制器
        if isinstance(controller, DeviceJdzhp02qs):
